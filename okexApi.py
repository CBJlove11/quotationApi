...
def get_restApi(db, cursor,instId,bar):
    ...
    sql = "insert into " + tableName + "(`symbol`,`interval`,`datetime`,`open_price`,`high_price`,`low_price`,`close_price`,`vol`,`volCcy`)" \
       ...
       " vol = values(vol), " \
       " volCcy = values(volCcy) "
    cursor.execute(query_Sql)
    rest = cursor.fetchone()
    if rest != None:
        openTime = rest[0] - interval_ms
        print("捕获数据库时间戳！")
    # 获取当前时间
    min_time = time.strftime("%Y-%m-%d %H:%M", time.localtime()) + ":00"
    minTime = int(time.mktime(time.strptime(min_time, "%Y-%m-%d %H:%M:%S"))) * 1000
    pdTime = minTime + 60000
    while True:
        # 判断是否满足条件进行请求
                ...
                try:
                    ...
                    while len(data) == 0:
                        ...
                        rp_1 = response.get('https://www.okx.com/api/v5/market/candles',
                                            headers=headers,
                                            params=params)
                        print("之前请求过早，服务器尚未有数据传输，先隔一秒再次进行请求：", instId, " bar=", bar, "  optime=", openTime,
                              "  endtime=none",
                              "url=https://www.okx.com/api/v5/market/candles?instId=", instId, "&bar=", bar, "&before=",
                              openTime)
                        data = json.loads(str(rp_1.text))["data"]
                    tmp_endTime = 0
                    for i in range(len(data)):
                        ...
                        if tmp_endTime == 0:
                            ...
                            tmp_endTime = int(data[i][0])
                    pdTime = pdTime + 60000
                    openTime = tmp_endTime - interval_ms
                    response.close()
                except Exception as e:
                    response.close()
                    time.sleep(5)
